...
function StartGraph(mavData, graphType, kernelName, lmemName, bankList, bankName, onLineMappingRequest, headerDiv) {
    "use strict";
	var curFile = null;
    ...
    function addClickFunctions(graph) {
        ...
        var clusters = graph.selectAll("g.cluster rect")
            .on('click', function (d) {
                ...
                if (clickDown == d) {
                    clickDown = null;
                } else if (flattenedNodes[d].type == "memsys" || flattenedNodes[d].type == "bank") {
                    highlightNodes(d, graph);
                    changeDivContent(0, detailTable(d));
                    clickDown = d;
                }
    ...
    function findFile(index) {
        ...
        Object.keys(mavData.fileIndexMap).forEach(function (fi) {
            if (mavData.fileIndexMap[fi] == index) filename = getFilenameFromPath(fi);
        });
        return filename;
    }
    ...
    function addHighlighting(graph) {
        ...
        var clusterHighlights = graph.selectAll("g.cluster rect")
            .on('mouseover', function (d) {
				if(flattenedNodes[d] == null) { return; }
                if (flattenedNodes[d].type == "memsys" || flattenedNodes[d].type == "bank") {
                    highlightNodes(d, graph);
                }
    ...
    function highlightNodes(d, graph) {
        ...
        // Highlight nodes
        var connectedNodes = graph.selectAll("g.node rect, g.node circle, g.node polygon, g.cluster rect")
            .filter(function (n) {
                if (associatedNodes.indexOf(n) == -1) return false;
                else if (flattenedNodes[d].type == "bb" || flattenedNodes[d].type == "memtype") return false;
                else return true;
            })
            .style("stroke-width", 3)
            .style("stroke", highlightColor);
    ...
    function setupSPG(graphType) {
        ...
        panelWidth = $(GID)[0].getBoundingClientRect().width - 2*offsetX;
        panelHeight = $(GID)[0].getBoundingClientRect().height - 2*offsetY;

        graphWidth = spgGroup[0][0].getBoundingClientRect().width + 2 * offsetX;
        graphHeight = spgGroup[0][0].getBoundingClientRect().height + 2 * offsetY;
        zoomFitScale = Math.min(panelWidth/graphWidth, panelHeight/graphHeight);

        offsetX += panelWidth/2 - graphWidth/2 * zoomFitScale; // Offset to center the graph in the middle
        offsetY += panelHeight/2 - graphHeight/2 * zoomFitScale; // Offset to center the graph in the middle

        // Add zoom and drag
        zoom = d3.behavior.zoom().on("zoom", function () {
            var x = d3.event.translate[0] + offsetX; // + panelWidth/2 - graphWidth/2 * zoomFitScale;
            ...
            if (flattenedNodes.hasOwnProperty(getUniqueNodeName(lnk.from)) && flattenedNodes.hasOwnProperty(getUniqueNodeName(lnk.to))) {
                ...
                if (graphType == "LMEM") {
                    ...
                    if ((flattenedNodes[getUniqueNodeName(lnk.to)].type == "inst" && flattenedNodes[getUniqueNodeName(lnk.from)].type == "memsys") ||
                        (flattenedNodes[getUniqueNodeName(lnk.to)].name == "loop" && flattenedNodes[getUniqueNodeName(lnk.from)].name == "loop end")) {
    ...
    function verifyDetails(a, b) {
      ...
      else if (akeys.length != bkeys.length) return false;
	...
	function changeDivContent(idx, detailsArray) {
	...
	function syncEditorPaneToLine( line, filename ) {
		if(onLineMappingRequest != null) {
			onLineMappingRequest(line, filename);
		}
