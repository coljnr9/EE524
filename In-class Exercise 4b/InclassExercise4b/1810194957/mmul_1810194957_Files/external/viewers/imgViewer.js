...
//IMAGE VIEWER:
//---------------
function ImageViewer(parent, isFromBufferViewer, closingButtonElement){
	var self = this;
	this.container = parent;
	this.isFromBufferViewer = isFromBufferViewer;
	this.closingButtonElement = closingButtonElement;
	this.mainImageInitializationInfo = null;
	...
	$(parent).find('.viewer-showAsBuffer').click(function(){
		...
		var bufferViewer = new BufferViewer(containerDiv, true, overlayDiv.closeButton);
		...
		for(var i=0; i<self.activePreviews.length; i++){
			...
			$.ajax({
				...
				success: function (bitmapPath) {
					bufferViewer.AddBuffer(title, bitmapPath, 'imageAsBuffer');
				},
	...
	$(parent).find('.viewer-compare').click(function(){
		var overlayDiv = openOverlayLayout('600px','300px', true, null, self.container);
		...
		for(var i=0; i<self.comparablesList.length; i++){
			...
			tableContent += '<tr><td class="comparableImageTitle' + additionalClass + '" ' +
										...
										'data-typeidentifier = "' + comparableImage.typeIdentifier + '" ' +
										'data-info = "' + comparableImage.info +
										'">' + comparableImage.name + '</td>' + 
										
										'<td>' + comparableImage.typeIdentifier + '</td>' + 
									'</tr>';
		}
		...
		tableContainer.innerHTML = '<table class="newAnalysisInputTable" style="position: relative; width: 100%; margin-left: 0px;">' + 
											  '<thead><tr style="outline: thin solid;"><td>Variable Name</td><td>Type</td></tr></thead><tbody>' + tableContent + '</tbody></table>';
		...
		$(tableContainer).find('.comparableImageTitle').click(function(){
			if($(this).hasClass('comparableImageTitle-selected')){
				...
				var requestStr = 'ImageViewer?add=' + data.name + '&' + data.path + '&' + data.typeidentifier + '&' + data.info;
...
ImageViewer.prototype.addComparableImage = function (path, name, typeIdentifier, info){
	var newComparable = 
	{
		"path": path,
		"name": name,
...
ImageViewer.prototype.popupViewer = function() {
	var varialesArray = [];
	
	//get main buffer data:
	var previewInstance = this.activePreviews[0];
	if(this.mainImageInitializationInfo != null){
		varialesArray.push(this.mainImageInitializationInfo);
	}
	else{
		alert('Failed to popup viewer: missing data');
		return;
	}
	
	//get the comparables:
	for(var i=0; i<this.comparablesList.length; i++){
		var comparableImage = this.comparablesList[i];
		var variableAjax;

		try{
			variableAjax = {
				"dataType": "image",
				"path": comparableImage.path,
				"name": comparableImage.name,
				"typeIdentifier": comparableImage.typeIdentifier,
				"info": comparableImage.info
			};
		}
		catch(error){
			variableAjax = {
                "dataType": "image",
				"path": comparableImage.path,
				"name": comparableImage.title
			};
		}
		
		//is it displayed?
		if(this.IsImageDisplayed(comparableImage.name)){
...
function ImagePreview(imgSrc, title, parent, syncFunction, mainViewer){
	...
	if(mainViewer.isFromBufferViewer == false){
	$.ajax({
			url:  'ImageViewer?canSave=' + self.title,
			type: "POST",
			async: true,
			dataType: "json",
			success: function (data) {
				...
				//add a saveAs button:
				self.saveAsImg = document.createElement('img');
				parent.appendChild(self.saveAsImg);
				self.saveAsImg.style.position = 'absolute';
				self.saveAsImg.style.top = '45px';
				self.saveAsImg.style.right = '9px';
				self.saveAsImg.style.width = '16px';
				self.saveAsImg.style.height = '16px';
				self.saveAsImg.style.zIndex = '999';
				self.saveAsImg.style.cursor = 'pointer';
				self.saveAsImg.title = 'save image';
				self.saveAsImg.src = filesBaseDir + '/resources/save.png';

				self.saveAsImg.onclick = function(){
					...
					if(data.canSelectFile == false){
						...
						var bmpSpan = document.createElement('span');
						bmpSpan.innerHTML = 'BMP';
						bmpSpan.className = 'intelLinkHoverColor';
						overlayDiv.appendChild(bmpSpan);
						
						var imgSpan = document.createElement('span');
						imgSpan.innerHTML = 'Binary';
						imgSpan.className = 'intelLinkHoverColor';
						imgSpan.style.marginLeft = '20px';
						overlayDiv.appendChild(imgSpan);
						
						bmpSpan.onclick = function(){ saveBufferAsFile('.bmp'); }
						imgSpan.onclick = function(){ saveBufferAsFile('.img'); }
						...
						function saveBufferAsFile(extension){
							...
							$.ajax({
								url:  'ImageViewer?saveAs_withoutFileSelection=' + self.title + '&' + extension,
								type: "POST",
								async: true,
								dataType: "text",
								success: function (exportPath) {
									$(savingOverlay).find('.overlayDivCloseButton')[0].style.visibility = '';
									savingOverlay.titleElement.innerHTML = 'Image saved successfully to:';
								...
								error: function(jqxhr, statusText, errorThrown){
									$(savingOverlay).find('.overlayDivCloseButton')[0].style.visibility = '';
									savingOverlay.titleElement.innerHTML = 'Image saving failed:<br/>' + errorThrown;
					...
					if(data.canSelectFile == true){
						$.ajax({
							url:  'ImageViewer?saveAs_withFileSelection=' + self.title,
							type: "POST",
							async: true,
							dataType: "text",
							success: function (data) {},
